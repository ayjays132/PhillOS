...
export const AgentConsole: React.FC = () => {
  ...
  return (
    <GlassCard className="flex flex-col h-full !p-0">
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-white/10 bg-black/10 flex gap-2">
        <input
          type="text"
          value={input}
          onChange={e => setInput(e.target.value)}
          className="flex-grow p-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none"
          placeholder="Ask the Agent..."
        />
        <button
          type="submit"
          disabled={isProcessing}
          className="px-4 py-2 bg-purple-600/80 hover:bg-purple-500/80 rounded-lg disabled:opacity-50"
        >
          Run
        </button>
      </form>

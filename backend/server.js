...
async function loadAIConfig() {
  try {
    const row = (
      await query("SELECT value FROM preferences WHERE key='ai_config'")
    )[0];
...
async function loadTheme() {
  try {
    const row = (
      await query("SELECT value FROM preferences WHERE key='theme'")
    )[0];
    if (row && (row.value === "light" || row.value === "dark"))
      return row.value;
  } catch (err) {
...
async function loadCursor() {
  try {
    const row = (
      await query("SELECT value FROM preferences WHERE key='cursor'")
    )[0];
    if (row && (row.value === "light" || row.value === "dark"))
      return row.value;
  } catch (err) {
...
const PHONE_BRIDGE_URL =
  process.env.PHONE_BRIDGE_URL || "http://localhost:3002";
...
app.use("/phonebridge", checkToken, (req, res) => {
  ...
  const target = new URL(
    req.originalUrl.replace(/^\/phonebridge/, ""),
    PHONE_BRIDGE_URL,
  );
  const body =
  ...
  const attempt = (n) => {
    ...
    proxyReq.on("error", (err) => {
      ...
      if (!res.headersSent)
        res.status(502).json({ error: "phone bridge unreachable" });
    });
    if (body) {
      proxyReq.write(body);
      proxyReq.end();
...
app.post("/api/converseai", async (req, res) => {
  ...
  try {
    ...
    if (!converseSession) {
      converseSession = await mod.createCloudChatSession(
        "gemini",
    ...
    let reply = "";
    for await (const chunk of mod.sendMessageStream(
      converseSession,
      finalMsg,
    )) {
      reply += chunk;
...
app.get("/api/inboxai/messages", async (req, res) => {
  const messages = await query(
    "SELECT id, sender as 'from', subject, body FROM emails",
  );
  res.json({ messages });
});
...
app.get("/api/inboxai/messages-sorted", async (req, res) => {
  const messages = await query(
    "SELECT id, sender as 'from', subject, body, score FROM emails ORDER BY score DESC",
  );
  res.json({ messages });
});
...
app.post("/api/inboxai/summary", async (req, res) => {
  const { id } = req.body || {};
  const row = (
    await query("SELECT body FROM emails WHERE id=?", [Number(id)])
  )[0];
...
app.post("/api/inboxai/score", async (req, res) => {
  ...
  await execute("UPDATE emails SET score=? WHERE id=?", [
    Number(score),
    Number(id),
  ]);
  res.json({ success: true });
...
app.post("/api/inboxai/reply", async (req, res) => {
  ...
  await execute("INSERT INTO emails(sender,subject,body) VALUES(?,?,?)", [
    "me@example.com",
    `Re:${id}`,
    text,
  ]);
...
app.get("/api/weblens/summarize", async (req, res) => {
  ...
  try {
    ...
    const citations = citationsRaw.map((c, i) => ({
      ...c,
      verified: verified[i],
    }));
...
async function detectScenes(file) {
  ...
  const times = await new Promise((resolve) => {
    ...
    p.on("close", () => {
      ...
      resolve(
        matches.map((m) => parseFloat(m[1])).filter((n) => !Number.isNaN(n)),
      );
    });
    p.on("error", () => resolve([]));
  });
  ...
  return {
    duration,
    chapters: times.map((t, i) => ({ start: t, title: `Scene ${i + 1}` })),
  };
}
...
async function createRecap(file) {
  ...
  return new Promise((resolve) => {
    const args = ["-i", file, "-t", "15", "-c", "copy", outFile, "-y"];
    const p = spawn("ffmpeg", args);
...
app.get("/api/spacemanager/groups", (req, res) => {
  ...
  const groups = Array.from(new Set(snap.map((w) => w.group ?? 0))).sort(
    (a, b) => a - b,
  );
  res.json({ groups });
});
...
app.post("/api/login", (req, res) => {
  ...
  try {
    const hash = getUserHash(String(username));
...
app.post("/api/brainpad/entries", async (req, res) => {
  ...
  await execute("INSERT INTO notes(content, created_at) VALUES(?, ?)", [
    text,
    Date.now(),
  ]);
  res.json({ success: true });
...
export default app;
export function setThreatScore(score) {
  threatScore = score;
}
export function setThreatPredictScore(score) {
  threatPredictScore = score;
}
export {
  sanitizeUserPath,
  sanitizeStoragePath,

...
2. Boot from the USB drive. The browser loads all core assets and API responses into the cache so subsequent boots work completely offline. The kernel re-checks `offline.cfg` on every boot so you can toggle offline mode by editing the file. Create `gpu.cfg` to override the detected GPU with `nvidia`, `amd`, `intel`, `none` or `auto`.

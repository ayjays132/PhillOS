...
class SecureCoreService {
  ...
  async getThreatPrediction() {
    try {
      const res = await fetch('/api/securecore/threatpredict');
      if (!res.ok) return { score: 0 };
      return await res.json();
    } catch {
      return { score: 0 };
...
agentOrchestrator.registerAction('secure.get_threat_predict', () => secureCoreService.getThreatPrediction());
